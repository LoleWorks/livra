...
export async function openGoogleMaps(lat: number, lng: number, label: string): Promise<void> {
  const enc = encodeURIComponent(label)
  const urls = Platform.OS === 'android'
    ? [`google.navigation:q=${lat},${lng}`, `https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}&travelmode=driving`]
    : [`comgooglemaps://?daddr=${lat},${lng}&directionsmode=driving`, `https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}&travelmode=driving`]
  for (const url of urls) {
    const ok = await Linking.canOpenURL(url).catch(() => false)
    if (ok) { await Linking.openURL(url); return }
  }
  Alert.alert('Google Maps indisponibil', 'Instalează Google Maps și încearcă din nou.')
}
...
export async function openWaze(lat: number, lng: number): Promise<void> {
  ...
  Alert.alert('Waze indisponibil', 'Instalează Waze și încearcă din nou.')
}
