...
- Shows: customer name, address, status badge, progress through route, driver location on mini-map, estimated arrival time
- Brand colors: orange `#ff5c2c` (not purple), dark `#161513`
- App download CTA at bottom: links to App Store (iOS), Google Play (Android), or `/app` (desktop)
...
- Supabase realtime subscription on the stop row for live status updates
...
### Location Tracking
- Background task defined in `src/lib/locationTask.ts` using `expo-task-manager`
- Task name: `livra-location-task`
...
- Background service notification: orange `#FF5C2C`, text "Transmitere locatie GPS activa"
- The background task uses its own minimal Supabase client (different JS context from main app)

### Screens

**LoginScreen** — PIN pad, queries `livra_drivers` by PIN, saves `driver_id` to storage
...
**AfterNavigationScreen** — Shown after driver returns from nav app. Buttons: "Am ajuns" (goes to success flow with signature) or "Nereusit" (goes to FailureReasonScreen)

**FailureReasonScreen** (`src/screens/FailureReasonScreen.tsx`) — Failure reason picker:
- Reasons list: Adresa incorecta, Client absent, Refuz primire, Parcel deteriorat, Usa inchisa cu clanta de siguranta, Doar coletul de pe usa, Altul
- On confirm: updates `livra_route_stops` status=failed, inserts to `livra_attention_items`, logs event, navigates to next stop or completes route

**SummaryScreen** — End-of-route summary

**ProfileScreen** — Driver profile, logout button

### Completing a Stop (success flow)
1. `AfterNavigationScreen` -> tap "Am ajuns"
...
6. Navigates to next pending stop or completes route
...
**Stack:** React Native + Expo  
Separate from the driver app. For end customers to track their deliveries.

### Screens
- `HomeScreen` — main dashboard
- `OrdersScreen` — list of customer orders
- `DeliveryDetailScreen` — individual delivery details
- `TrackScreen` — live map tracking of delivery
- `LocationsScreen` / `SetLocationScreen` — delivery location management
- `ProfileScreen` — customer profile + auth
- `LoginScreen` — customer login

### Context
- `AuthContext` — customer auth state
- `OrdersContext` — orders data
...
Failed deliveries needing dispatcher follow-up. Columns: `id, stop_id, customer, phone, address, driver_name, fail_reason, status ('open'|'resolved'), failed_at, created_at`
...
### `livra_customers`
End customers with the customer app installed. Columns: `id, phone, push_token, ...`
...
- Push title: "Soferul este aproape! [truck emoji]"
...
7. Edge function: if customer has push token -> Expo push; else -> Twilio SMS
...
- `livra.loleworks.com` frontend must be rebuilt + redeployed after any `src/` changes
