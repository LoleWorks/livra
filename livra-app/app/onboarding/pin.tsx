...
export default function OnboardingPin() {
  ...
  const handleSave = async () => {
    ...
    await supabase.from('livra_customers').update({ pins: [pin] }).eq('id', user!.id)
    ...
    <View style={[styles.root, { paddingTop: insets.top }]}>
      <ScreenHeader leftIcon="back" transparent rightSlot={
        <TouchableOpacity onPress={() => router.replace('/(tabs)/home')}>
          <Text style={styles.skip}>Mai târziu</Text>
        </TouchableOpacity>
      } />
      ...
      <View style={styles.map}>
        ...
        {/* Pinned address bubble — native only (web uses the fixed pin overlay) */}
        ...
        {/* Centre pin — native only; web version renders its own fixed pin */}
      ...
      <View style={[styles.sheet, { paddingBottom: Math.max(T.space.xl, insets.bottom + T.space.md) }]}>
        ...
        <Button label={loading ? 'Se salvează…' : 'Salvează și termină'} variant="accent" onPress={handleSave} disabled={loading} />
      </View>
    </View>
  )
}
