    ...
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: Math.max(T.space.xl, insets.bottom + T.space.md) }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          {/* Pickup address for on-demand */}
          {(stop as any).pickup_address && (
            <View style={styles.detailRow}>
              <Feather name="upload" size={14} color={T.color.inkMuted} />
              <View style={{ flex: 1 }}>
                <Text style={styles.detailLabel}>RIDICARE</Text>
                <Text style={styles.detailText}>{(stop as any).pickup_address}</Text>
              </View>
            </View>
          )}
          <View style={styles.detailRow}>
            ...
            <View style={{ flex: 1 }}>
              {(stop as any).pickup_address && <Text style={styles.detailLabel}>LIVRARE</Text>}
          ...
          {(stop as any).price_estimation && (
            <View style={styles.priceStrip}>
              <Text style={styles.priceStripLabel}>ESTIMARE COST</Text>
              <Text style={styles.priceStripValue}>{(stop as any).price_estimation} MDL</Text>
            </View>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  detailRow:     { flexDirection: 'row', alignItems: 'flex-start', gap: T.space.xs, paddingVertical: 4 },
  detailLabel:   { fontSize: T.size.micro, fontWeight: T.weight.bold, color: T.color.inkSubtle, letterSpacing: 0.5, marginBottom: 2 },
  detailText:    { fontSize: T.size.bodySm, color: T.color.inkMuted, flex: 1, lineHeight: 20 },
  notesBox:      { flexDirection: 'row', gap: T.space.xs, backgroundColor: T.color.warningBg, borderRadius: T.radius.sm, padding: T.space.sm, borderLeftWidth: 3, borderLeftColor: T.color.warning, marginTop: 4 },
  notesText:     { flex: 1, fontSize: T.size.caption, color: '#92400e', lineHeight: 18 },
  priceStrip:    { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: T.color.bg, padding: T.space.sm, borderRadius: T.radius.md, marginTop: T.space.sm },
  priceStripLabel: { fontSize: T.size.micro, fontWeight: T.weight.bold, color: T.color.inkSubtle },
  priceStripValue: { fontSize: T.size.bodySm, fontWeight: T.weight.bold, color: T.color.ink, fontFamily: T.font.mono },
