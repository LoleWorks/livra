...
export default function EditPin() {
  ...
  const pin = customer?.pins?.find(p => p.id === id)

  const [name,        setName]        = useState(pin?.name ?? '')
  const [search,      setSearch]      = useState(pin?.address ?? '')
  const [suggestions, setSuggestions] = useState<Suggestion[]>([])
  const [searching,   setSearching]   = useState(false)
  const [pinnedAddr,  setPinnedAddr]  = useState(pin?.address ?? '')
  const [flyTo,       setFlyTo]       = useState<{ lat: number; lng: number } | null>(
    pin?.lat && pin?.lng ? { lat: pin.lat, lng: pin.lng } : null
  )
  const [pinLat,      setPinLat]      = useState<number | null>(pin?.lat ?? null)
  const [pinLng,      setPinLng]      = useState<number | null>(pin?.lng ?? null)
  ...
  const pinnedAddrRef  = useRef(pin?.address ?? '')
  ...
  const handleSave = async () => {
    ...
    const updated: Pin = { id: id!, name: name.trim(), address: addr, lat: pinLat, lng: pinLng, primary: pin?.primary ?? false }
    const pins = (customer?.pins ?? []).map(p => p.id === id ? updated : p)
  ...
  const handleSetPrimary = async () => {
    const pins = (customer?.pins ?? []).map(p => ({ ...p, primary: p.id === id }))
    await supabase.from('livra_customers').update({ pins }).eq('id', user!.id)
    await refreshCustomer()
  }

  const handleDelete = () => {
    Alert.alert('Șterge locația', `Ești sigur că vrei să ștergi "${pin?.name}"?`, [
      { text: 'Anulează', style: 'cancel' },
      {
        text: 'Șterge', style: 'destructive',
        onPress: async () => {
          setDeleting(true)
          const remaining = (customer?.pins ?? []).filter(p => p.id !== id)
          if (remaining.length > 0 && !remaining.some(p => p.primary)) {
            remaining[0].primary = true
          }
          await supabase.from('livra_customers').update({ pins: remaining }).eq('id', user!.id)
          await refreshCustomer()
          setDeleting(false)
          router.back()
        },
  ...
  if (!pin) {
    return (
      <View style={[styles.root, { paddingTop: insets.top }]}>
        <ScreenHeader leftIcon="close" title="Locație" />
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
          <Text style={{ color: T.color.inkMuted }}>Locație negăsită.</Text>
        </View>
      </View>
    )
  }
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      <ScreenHeader leftIcon="close" title="Editează locația" />
        ...
        <View style={styles.searchWrap}>
          <View style={styles.searchBar}>
            ...
            <TextInput
              ...
              value={search}
              onChangeText={t => { setSearch(t); setPinnedAddr('') }}
      ...
      <View style={[styles.sheet, { paddingBottom: Math.max(T.space.xl, insets.bottom + T.space.md) }]}>
        ...
        <View style={styles.actions}>
          {!pin.primary && (
            <TouchableOpacity style={styles.secondaryBtn} onPress={handleSetPrimary}>
              <Feather name="star" size={14} color={T.color.primary} />
              <Text style={styles.secondaryBtnText}>Setează ca primară</Text>
            </TouchableOpacity>
          )}
          <TouchableOpacity style={styles.deleteBtn} onPress={handleDelete} disabled={deleting}>
            <Feather name="trash-2" size={14} color="#ef4444" />
            <Text style={styles.deleteBtnText}>{deleting ? 'Se șterge…' : 'Șterge locația'}</Text>
          </TouchableOpacity>
        </View>

        <Button label={loading ? 'Se salvează…' : 'Salvează modificările'} variant="accent" onPress={handleSave} disabled={loading} />
      </View>
    </View>
  )
}
...
const styles = StyleSheet.create({
  ...
  secondaryBtn:    { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 10, borderRadius: T.radius.md, borderWidth: 1, borderColor: T.color.primary, backgroundColor: T.color.primaryLight },
  secondaryBtnText:{ fontSize: T.size.caption, fontWeight: T.weight.semibold, color: T.color.primary },
  deleteBtn:       { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 10, borderRadius: T.radius.md, borderWidth: 1, borderColor: '#fecaca', backgroundColor: '#fef2f2' },
  deleteBtnText:   { fontSize: T.size.caption, fontWeight: T.weight.semibold, color: '#ef4444' },
})
