...
function stepIndex(status: RouteStop['status']) {
  ...
  if (status === 'arriving' || status === 'transit') return 2
  if (status === 'dispatched' || status === 'preparing') return 1
  return 0 // pending
}
...
export default function TrackScreen() {
  ...
  const fetchDriver = useCallback(async (routeId: string) => {
    ...
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      <View style={styles.mapWrap}>
        <TrackMap
          ...
          destLat={stop.lat}
          destLng={stop.lng}
          pickupLat={(stop as any).pickup_lat}
          pickupLng={(stop as any).pickup_lng}
        />
      ...
      <View style={[styles.sheet, { paddingBottom: Math.max(T.space.xl, insets.bottom + T.space.md) }]}>
        ...
        {(stop as any).pickup_address && (
          <View style={styles.addressBox}>
            <View style={styles.addrDotWrap}>
              <View style={[styles.addrDot, { backgroundColor: '#000' }]} />
              <View style={styles.addrLine} />
              <View style={[styles.addrDot, { backgroundColor: T.color.primary }]} />
            </View>
            <View style={{ flex: 1, gap: T.space.sm }}>
              <View>
                <Text style={styles.addrLabel}>RIDICARE</Text>
                <Text style={styles.addrText} numberOfLines={1}>{(stop as any).pickup_address}</Text>
              </View>
              <View>
                <Text style={styles.addrLabel}>LIVRARE</Text>
...
const styles = StyleSheet.create({
  ...
  addressBox:         { flexDirection: 'row', gap: T.space.md, backgroundColor: T.color.bg, borderRadius: T.radius.lg, padding: T.space.md, marginVertical: T.space.xs },
  addrDotWrap:        { alignItems: 'center', width: 10, paddingVertical: 4 },
  addrDot:            { width: 8, height: 8, borderRadius: 4 },
  addrLine:           { width: 1, flex: 1, backgroundColor: T.color.border, marginVertical: 4 },
  addrLabel:          { fontSize: T.size.micro, color: T.color.inkSubtle, fontWeight: T.weight.bold, letterSpacing: 0.5, marginBottom: 2 },
  addrText:           { fontSize: T.size.bodySm, color: T.color.ink, fontWeight: T.weight.medium },
})
