...
function formatAddr(r: NominatimResult): { main: string; sub: string } {
  const a = r.address
  if (a) {
    ...
    const cityPart = district && city ? `${district}, ${city}` : city || district
    if (street) return { main: street, sub: cityPart }
  }
...
type LivraPin = { id: string; name: string; address: string | null; lat: number | null; lng: number | null; primary: boolean }
type LivraCustomer = { name: string | null; pins: LivraPin[]; time_window_start: string | null; time_window_end: string | null }

function normalizePhone(raw: string): string | null {
  const d = raw.replace(/[\s\-().+]/g, '')
  if (/^373\d{7,8}$/.test(d)) return d              // 373XXXXXXXX stored without +
  if (/^0\d{7,8}$/.test(d))   return '373' + d.slice(1) // 069XXXXXXX → 37369XXXXXXX
  if (/^\d{7,8}$/.test(d))    return '373' + d      // bare digits
  return null
}

function useLivraCustomer(phone: string) {
  const [customer, setCustomer] = useState<LivraCustomer | null>(null)
  ...
  useEffect(() => {
    ...
    timer.current = setTimeout(async () => {
      const { data } = await supabase.rpc('lookup_customer_by_phone', { p_phone: normalized })
...
const EMPTY: Form = {
  customer: '', phone: '', address: '', addr_lat: '', addr_lng: '', notes: '',
...
export default function SalesNewOrder() {
  ...
  const livraCustomer = useLivraCustomer(form.phone)
  const [pinDropdownOpen, setPinDropdownOpen] = useState(false)

  function autofill() {
    if (!livraCustomer) return
    setForm(f => ({
      ...f,
      customer:          livraCustomer.name            ?? f.customer,
      time_window_start: livraCustomer.time_window_start ?? f.time_window_start,
      time_window_end:   livraCustomer.time_window_end   ?? f.time_window_end,
    }))
    if (livraCustomer.pins.length > 0) setPinDropdownOpen(true)
  }
  const [addrOpen, setAddrOpen] = useState(false)
  const addrRef = useRef<HTMLDivElement>(null)
  const { results: addrSuggestions, loading: addrLoading } = useAddressSuggestions(addrOpen && !pinDropdownOpen ? form.address : '')
  ...
  async function submit(e: React.FormEvent) {
    ...
    const { error: dbErr } = await supabase.from('livra_deliveries').insert({
      ...
      lat:                 form.addr_lat ? parseFloat(form.addr_lat) : null,
      lng:                 form.addr_lng ? parseFloat(form.addr_lng) : null,
            ...
            <Field label="Telefon" icon={Phone} required>
              ...
              {livraCustomer && (
                <button
                  type="button"
                  onClick={autofill}
                  className="mt-2 w-full flex items-center gap-2.5 px-3 py-2 bg-orange-50 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-800/50 rounded-lg hover:bg-orange-100 dark:hover:bg-orange-950/50 transition-colors text-left"
                >
                  <div className="w-6 h-6 rounded-full bg-brand-orange flex items-center justify-center flex-shrink-0">
                    <span className="text-[10px] font-bold text-white">L</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-[12px] font-semibold text-orange-700 dark:text-orange-300 truncate">
                      {livraCustomer.name ?? 'Cont Livra'} — are cont Livra
                    </div>
                    <div className="text-[11px] text-orange-500/80 dark:text-orange-400/70">
                      Click pentru a completa automat și a alege adresa de livrare
                    </div>
                  </div>
            ...
            <Field label="Adresa de livrare" icon={MapPin} required>
              <div ref={addrRef} className="relative">
                <input type="text" className={inputCls} placeholder="ex. str. Ismail 12, Chișinău"
                  ...
                  onChange={e => { set('address', e.target.value); set('addr_lat', ''); set('addr_lng', ''); setPinDropdownOpen(false); setAddrOpen(true) }}
                  onFocus={() => { if (!pinDropdownOpen) setAddrOpen(true) }} />
                {addrLoading && <Loader2 size={13} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-400 animate-spin" />}

                {/* Livra saved locations dropdown */}
                {pinDropdownOpen && livraCustomer && livraCustomer.pins.length > 0 && (
                  <ul className="absolute z-50 left-0 right-0 top-full mt-1 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-lg overflow-hidden">
                    <li className="px-3 py-1.5 border-b border-zinc-100 dark:border-zinc-800">
                      <span className="text-[11px] font-semibold text-orange-500 uppercase tracking-wide">Locații salvate în Livra</span>
                    </li>
                    {livraCustomer.pins.map(p => {
                      const addrText = p.address ?? (p.lat != null ? `${p.lat.toFixed(5)}, ${p.lng?.toFixed(5)}` : 'Pin GPS')
                      return (
                        <li key={p.id}>
                          <button type="button"
                            ...
                            onMouseDown={e => {
                              e.preventDefault()
                              set('address', addrText)
                              set('addr_lat', p.lat != null ? String(p.lat) : '')
                              set('addr_lng', p.lng != null ? String(p.lng) : '')
                              setPinDropdownOpen(false)
                            }}>
                            <div className="flex items-center gap-2">
                              <div className="w-6 h-6 rounded-full bg-orange-100 dark:bg-orange-950/50 flex items-center justify-center flex-shrink-0">
                                <MapPin size={11} className="text-brand-orange" />
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1.5">
                                  <span className="text-[13px] font-medium text-zinc-800 dark:text-zinc-200">{p.name}</span>
                                  {p.primary && <span className="text-[9px] font-bold bg-orange-100 text-orange-600 dark:bg-orange-900/40 dark:text-orange-400 px-1.5 py-0.5 rounded uppercase tracking-wide">Primară</span>}
                                </div>
                                <div className="text-[11px] text-zinc-400 truncate">{addrText}</div>
                              </div>
                            </div>
                          </button>
                        </li>
                    ...
                    <li className="border-t border-zinc-100 dark:border-zinc-800">
                      <button type="button"
                        className="w-full text-left px-3 py-2 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors"
                        onMouseDown={e => { e.preventDefault(); setPinDropdownOpen(false); setAddrOpen(true) }}>
                        <div className="flex items-center gap-2 text-zinc-500 dark:text-zinc-400">
                          <Search size={11} />
                          <span className="text-[12px]">Caută altă adresă…</span>
                        </div>
                      </button>
                    </li>
                  </ul>
                ...
                {addrOpen && !pinDropdownOpen && addrSuggestions.length > 0 && (
                  <ul className="absolute z-50 left-0 right-0 top-full mt-1 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-lg overflow-hidden">
                    {addrSuggestions.map(r => {
                      ...
                      return (
                        <li key={r.place_id}>
                          <button type="button"
                            className="w-full text-left px-3 py-2.5 hover:bg-orange-50 dark:hover:bg-orange-950/30 transition-colors"
                            onMouseDown={e => { e.preventDefault(); set('address', full); set('addr_lat', r.lat); set('addr_lng', r.lon); setAddrOpen(false) }}>
